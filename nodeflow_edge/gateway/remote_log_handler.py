"""
Nodeflow Edge Remote Log Handler

A Python logging.Handler that buffers log records and periodically publishes
them to Nodeflow Cloud via MQTT on the `v1/gateway/logs` topic.
...
log = logging.getLogger("nodeflow_edge.remote_log_handler")


class RemoteLogHandler(logging.Handler):
    """
    Logging handler that buffers log records and publishes batches
    to Nodeflow Cloud via MQTT.
    """
    ...
    def emit(self, record):
        ...
        if record.name.startswith("nodeflow_edge.mqtt_publisher"):
            return
        if record.name == "nodeflow_edge.remote_log_handler":
            return
