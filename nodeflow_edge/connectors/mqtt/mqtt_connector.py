...
from nodeflow_edge.connectors.mqtt.backward_compatibility_adapter import BackwardCompatibilityAdapter
from nodeflow_edge.gateway.constant_enums import Status
from nodeflow_edge.connectors.connector import Connector
from nodeflow_edge.connectors.mqtt.mqtt_decorators import CustomCollectStatistics
from nodeflow_edge.gateway.constants import DATA_RETRIEVING_STARTED, CONVERTED_TS_PARAMETER, RPC_DEFAULT_TIMEOUT
from nodeflow_edge.gateway.entities.converted_data import ConvertedData
from nodeflow_edge.gateway.statistics.decorators import CollectAllReceivedBytesStatistics
            ...
            for regex in topic_handlers:
                ...
                request_arguments = (found_device_name, found_attribute_names,
                                     lambda data, *args: self.notify_attribute(data, found_attribute_names,
                                                                               ...
                                                                               handler.get('qos', 0)))
                # Attribute requests require a ThingsBoard server connection (not available in Nodeflow Edge)
                self.__log.warning('Attribute requests are not supported in Nodeflow Edge (no TB server). Skipping.')
                break
            return True, content
