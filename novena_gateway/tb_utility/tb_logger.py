...
#     Adapted for Novena Gateway — ThingsBoard server dependencies removed.
#
...
def init_logger(gateway, name, level, enable_remote_logging=False, is_connector_logger=False,
    ...
    Novena Gateway version: local logging only (no remote TB server logging).
    """
...
class TbLogger(logging.Logger):
    ...
    def send_errors_if_needed(cls, gateway):
        # Novena Gateway: no remote error reporting to TB server
    ...
    def check_and_update_file_handlers_class_name(config):
        for handler_config in config.get('handlers', {}).values():
            if handler_config.get('class', '') == 'novena_gateway.tb_utility.tb_handler.TimedRotatingFileHandler':
                handler_config['class'] = 'novena_gateway.tb_utility.tb_rotating_file_handler.TimedRotatingFileHandler' # noqa
