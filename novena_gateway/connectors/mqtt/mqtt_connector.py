...
from novena_gateway.connectors.mqtt.backward_compatibility_adapter import BackwardCompatibilityAdapter
from novena_gateway.gateway.constant_enums import Status
from novena_gateway.connectors.connector import Connector
from novena_gateway.connectors.mqtt.mqtt_decorators import CustomCollectStatistics
from novena_gateway.gateway.constants import DATA_RETRIEVING_STARTED, CONVERTED_TS_PARAMETER, RPC_DEFAULT_TIMEOUT
from novena_gateway.gateway.entities.converted_data import ConvertedData
from novena_gateway.gateway.statistics.decorators import CollectAllReceivedBytesStatistics
            ...
            for regex in topic_handlers:
                ...
                request_arguments = (found_device_name, found_attribute_names,
                                     lambda data, *args: self.notify_attribute(data, found_attribute_names,
                                                                               ...
                                                                               handler.get('qos', 0)))
                # Attribute requests require a ThingsBoard server connection (not available in Novena Gateway)
                self.__log.warning('Attribute requests are not supported in Novena Gateway (no TB server). Skipping.')
                break
            return True, content
